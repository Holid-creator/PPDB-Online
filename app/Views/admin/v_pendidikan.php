        ...
        <tbody>
          ...
          foreach ($pendidikan as $data) : ?>
            <tr>
              ...
              <td>
                <button class="btn btn-info btn-sm btn-flat" data-toggle="modal" data-target="#edit<?= $data['id_pendidikan']; ?>"><i class="fas fa-edit"></i> Ubah</button>
                <button class="btn btn-danger btn-sm btn-flat" data-toggle="modal" data-target="#hapus<?= $data['id_pendidikan']; ?>"><i class="fas fa-trash"></i> hapus</button>
              </td>
            </tr>
          <?php endforeach ?>
        </tbody>
...
<?php foreach ($pendidikan as $data) : ?>
  <div class="modal fade" id="edit<?= $data['id_pendidikan']; ?>">
    <div class="modal-dialog">
      <div class="modal-content">
        <div class="modal-header">
          <h4 class="modal-title">Tambah Data pendidikan</h4>
        ...
        <div class="modal-body">
          <?= form_open('pendidikan/ubah/' . $data['id_pendidikan']) ?>
          <div class="form-group">
            <label for="pendidikan">pendidikan</label>
            <input type="text" class="form-control" name="pendidikan" id="pendidikan" value="<?= $data['pendidikan'] ?>">
          </div>
        </div>
        <div class="modal-footer justify-content-between">
          <button type="button" data-dismiss="modal" class="btn btn-danger btn-sm btn-flat">Tutup</button>
...
<?php foreach ($pendidikan as $data) : ?>
  <div class="modal fade" id="hapus<?= $data['id_pendidikan']; ?>">
    <div class="modal-dialog">
      <div class="modal-content">
        ...
        <div class="modal-body">
          Apakah Anda Yakin ingin Menghapusnya <?= $data['pendidikan']; ?>
        </div>
        <div class="modal-footer justify-content-between">
          <button type="button" data-dismiss="modal" class="btn btn-danger btn-sm btn-flat">Tidak</button>
          <a href="<?= base_url('pendidikan/hapus/' . $data['id_pendidikan']) ?>" class="btn btn-success btn-sm btn-flat">Ya, Hapus</a>
        </div>
      </div>
    </div>
  </div>
